...
call :LOG "===== HomeoCare AI Launch Started %DATE% %TIME% ====="
cls
echo.
echo  ================================================================
echo   HomeoCare AI  -  Homeopathic Medicine Encyclopedia v1.0
echo  ================================================================
echo.
echo  EDUCATIONAL DISCLAIMER:
echo  This is an educational research tool. Not medical advice.
echo  Mainstream reviews found no reliable evidence beyond placebo.
echo  ALWAYS consult a registered homeopath (BHMS/NCH) and your doctor.
echo  Never a substitute for vaccination or emergency care.
echo  EMERGENCY: 112 (India) / 999 (UK) / 911 (US)
echo.
echo  ================================================================
echo.
...
echo.
echo  [STEP 6/7]  Checking port 5125...
call :LOG "Checking port 5125"
netstat -ano 2>nul | findstr ":5125 " >nul 2>&1
if %errorlevel%==0 (for /f "tokens=5" %%P in ('netstat -ano 2^>nul ^| findstr ":5125 "') do taskkill /PID %%P /F >nul 2>&1 & timeout /t 2 /nobreak >nul & echo              [ OK ]  Port freed.) else (echo              [ OK ]  Port 5125 available.)
echo.
echo  [STEP 7/7]  Starting HomeoCare AI...
call :LOG "Starting server on port 5125"
...
start "HomeoCare AI Server" cmd /k "echo HomeoCare AI Server - Keep This Window Open && "%VPYTHON%" "%BASEDIR%\server.py" --port 5125"
echo              Waiting for server...
set "SERVER_UP=false"
for /L %%i in (1,1,25) do (
    timeout /t 1 /nobreak >nul
    curl -s "http://localhost:5125/api/health" >nul 2>&1
...
echo.
echo  ================================================================
echo   HomeoCare AI IS RUNNING
echo  ================================================================
echo.
echo   Open in browser:   http://localhost:5125
echo   Mode: Online = %ONLINE%
echo   Security: AES-256-GCM API key encryption active
echo   Log: %LOGFILE%
echo.
echo  ================================================================
echo   DISCLAIMER: Educational tool only. NOT medical advice.
echo   No reliable evidence beyond placebo per mainstream reviews.
echo   Never a substitute for vaccination or emergency care.
echo   EMERGENCY: 112 (India) / 999 (UK) / 911 (US)
echo  ================================================================
echo.
...
goto :MENU
:QUIT
taskkill /FI "WINDOWTITLE eq HomeoCare AI Server*" /F >nul 2>&1
for /f "tokens=5" %%P in ('netstat -ano 2^>nul ^| findstr ":5125 "') do taskkill /PID %%P /F >nul 2>&1
